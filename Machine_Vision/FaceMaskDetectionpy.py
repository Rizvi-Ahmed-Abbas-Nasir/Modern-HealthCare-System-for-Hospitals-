...
def check_for_gpu(gpu_index=1):
    ...
    if gpus:
        ...
        try:
            ...
            gpu_config.set_logical_device_configuration(gpus[1],
                                                        [gpu_config.LogicalDeviceConfiguration(memory_limit=8000)])
...
dataset_folder = "face-mask-dataset4/Dataset/train/"
...
print(labels)
NO_OF_CLASSES = 2     
label_array = {'with_cap':0, 'without_cap':1}
...
save_model_path = "FaceMaskGPU3.h5"
    ...
    if results:
        ...
        if outputs.detections:
             ...
             for region in outputs.detections:   
                ...
                detect_class = model.predict(image_s)
                classes_x = np.argmax(detect_class, axis=1)
